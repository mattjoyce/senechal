...
class WithingsETL(HealthETL):
    """ETL process for Withings health data."""
    ...
    def identify_affected_periods(
        self, measurement_dates: List[datetime]
    ) -> Set[Tuple]:
        """
    ...
    def process_new_measurements(self, dry_run: bool = False) -> None:
        ...
        for measurement in new_measurements:
            try:
                ...
                logging.error(
                    f"Error parsing date for measurement {measurement['withings_id']}: {error}"
                )
    ...
    def process_period(
        ...
        senechal_db.commit()
        logging.info(
            f"Updated summaries for {period_type} period: {start_date} to {end_date}"
        )
