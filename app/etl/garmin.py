...
class GarminETL(HealthETL):
    ...
    def identify_summary_periods(self) -> Set[Tuple]:
        ...
        for table_name, period_type in PERIOD_TYPE_MAP.items():
            ...
            if not cursor.fetchone():
                logging.warning(f"Table {table_name} not found in Garmin summary database")
                continue
                ...
                try:
                    ...
                    if period_type == 'day':
                        ...
                        if period_start.month == 12:
                            period_end = date.replace(year=date.year+1, month=1, day=1)
                        else:
                            period_end = date.replace(month=period_start.month+1, day=1)
                        period_end = period_end - timedelta(microseconds=1)
                    elif period_type == 'year':
                        period_start = date.replace(hour=0, minute=0, second=0, microsecond=0)
                        period_end = date.replace(year=date.year+1, month=1, day=1) - timedelta(microseconds=1)
    ...
    def process_period(
        ...
        if summary_metrics:
            ...
            logging.info(f"Retrieved {len(summary_metrics)} metrics from Garmin summary database")
        else:
            logging.info(f"No summary data found for {period_type} from {start_date} to {end_date}, using raw data")
    ...
    def _get_summary_metrics(self, period_type: str, start_date: datetime, end_date: datetime) -> Dict:
        ...
        try:
            ...
            if not summary_table:
                logging.warning(f"No matching summary table for period type {period_type}")
                summary_db.close()
                return metrics
            ...
            """)
            if not cursor.fetchone():
                logging.warning(f"Table {summary_table} not found in Garmin summary database")
                summary_db.close()
                return metrics
            ...
            row = cursor.fetchone()
            if not row:
                logging.warning(f"No summary data found for {period_type} starting {start_date.date()}")
                summary_db.close()
                return metrics
    ...
    def _save_metrics(
        ...
        senechal_db.commit()
        logging.info(f"Saved {len(metrics)} metrics for {period_type} period: {start_date} to {end_date}")
