...
def main():
    ...
    parser.add_argument(
        "--since", type=str, help="Process data since a specific date (DD/MM/YYYY)"
    )
    parser.add_argument("--days", type=int, help="Process data for the last N days")
    ...
    try:
        if args.since:
            ...
            try:
                ...
                while current_date <= today:
                    affected_periods.update(
                        etl.identify_affected_periods([current_date])
                    )
            ...
            logging.info(
                f"Processing data for the last {args.days} days (since {start_date.date()})"
            )
